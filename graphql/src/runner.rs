...
use graph::prelude::futures03::stream::{FuturesOrdered, StreamExt};
    ...
    SM: SubscriptionManager,
{
    ...
    async fn execute(
        ...
        let mut query_res_futures = FuturesOrdered::new();
        ...
        for (bc, (selection_set, error_policy)) in by_block_constraint {
            ...
            query_res_futures.push_back(execute_query(
                query.clone(),
                Some(selection_set),
        ...
        let results: Vec<_> = query_res_futures.collect().await;
